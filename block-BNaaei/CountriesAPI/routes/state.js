...
router.post("/state", async (req, res) => {
  try {
    ...
    let updatecountry = await CSSCounterStyleRule.findByIdAndUpdate(
      country._id,
      {
...
router.get("/:countryname/states/ascending", async (req, res) => {
  try {
    ...
    let statesinAscending = await States.aggregate([
...
router.get("/:countryname/states/decending", async (req, res) => {
  try {
    let countryname = req.params.countryname;
    console.log(countryname);
    let statesinDecending = await statesinDecending.aggregate([
      { $match: { countryname: countryname } },
      { $sort: { name: -1 } },
    ]);
